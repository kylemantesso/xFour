 ...
 * x402 Agent SDK
 *
 * TypeScript SDK for AI agents and applications that consume paid APIs.
...
export type AgentClientConfig = {
  ...
  /**
   * API key issued by the gateway for authentication (agent/consumer key)
   */
  apiKey: string;
...
export function createAgentClient(config: AgentClientConfig): AgentClient {
  ...
  async function quoteFromResponse(
    ...
    return {
      ...
      amount: String(rawResult.amount ?? 0),
      network: rawResult.network ?? "mainnet",
    };
  }
  ...
  async function pay(paymentId: string): Promise<PayResult> {
    ...
    return {
      ...
      paymentId: rawResult.paymentId,
      invoiceId: rawResult.invoiceId,
      amount: rawResult.amount !== undefined ? String(rawResult.amount) : undefined,
      network: rawResult.network ?? undefined,
